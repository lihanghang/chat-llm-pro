...
def request_api(
    ...
    with open(filename, "rb") as f:
        res = requests.post(
            url=base_url + endpoint, headers=headers, data=f, params=query
        )
    ...
    try:
        if ext in [".doc", ".docx"]:
            ...
            request_api(
                base_url=base_url,
                endpoint=pdf2doc,
                filename=convert_pdf_dir,
                params=query,
                output_dir=store_origin_file_dir,
                output_format="zip",
            )
...
if __name__ == "__main__":
    ...
    for item in doc_obj[DocField.ITEMS.value]:
        doc_result.append(
            DocContent(
                item[DocField.TYPE.value],
                item[DocField.PAGE_NUMBER.value],
                item[DocField.TEXT.value],
            )
        )
